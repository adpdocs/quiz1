...
const Quiz = () => {
  ...
  return (
    <div className="max-w-md mx-auto p-4 md:p-6 lg:p-8 bg-white rounded-lg shadow-md border border-gray-200">
      {currentQuestion < questions.length ? (
        <div>
          <h2 className="text-xl font-bold mb-4 text-center">{questions[currentQuestion].question}</h2>
          <ul className="space-y-3">
            {questions[currentQuestion].options.map((option, index) => (
              <li key={index} className="mb-2">
                <button
                  className={`w-full p-3 text-lg font-medium border rounded-md bg-gray-100 hover:bg-gray-200 transition-colors ${
          ...
          <button
            className="w-full mt-4 p-3 text-lg font-semibold bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors"
